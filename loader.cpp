...
static MinimalMapResult MinimalManualMap(const uint8_t* dllData, size_t dllSize) {
    ...
    // ★ Fix: 验证 e_lfanew 不越界
    if (static_cast<size_t>(dos->e_lfanew) + sizeof(IMAGE_NT_HEADERS) > dllSize) return result;
    ...
    // --- 3. 复制 PE 头和区段 ---
    // ★ Fix: 验证 SizeOfHeaders 不越界
    DWORD headerSize = nt->OptionalHeader.SizeOfHeaders;
    if (headerSize > dllSize) return result;
    memcpy(imageBase, dllData, headerSize);

    auto* firstSection = IMAGE_FIRST_SECTION(nt);
    for (int i = 0; i < nt->FileHeader.NumberOfSections; i++) {
        if (firstSection[i].SizeOfRawData > 0) {
            // ★ Fix: 验证区段数据不越界 (PointerToRawData + SizeOfRawData)
            DWORD rawOff = firstSection[i].PointerToRawData;
            DWORD rawSize = firstSection[i].SizeOfRawData;
            if (static_cast<size_t>(rawOff) + rawSize > dllSize) return result;
            memcpy(imageBase + firstSection[i].VirtualAddress,
                   dllData + rawOff, rawSize);
        }
    }
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    ...
    // ★ Fix: 验证 originalSize 不超过实际缓冲区大小
    if (originalSize > encryptedPayloadSize) {
        return 2; // 数据头损坏, 原始大小超过负载范围
    }
