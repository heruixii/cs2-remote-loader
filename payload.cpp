...
// ★ v3.126d: 手动解析远程进程中的导入表 (IAT) — Process Hollowing 后 IAT 未初始化
//   遍历 import descriptor, 用 GetProcAddress 解析每个函数地址并写入远程 IAT。
static bool ResolveImportTable(HANDLE hProcess, void* remoteBase,
    ...
    for (DWORD idx = 0; ; idx++) {
        ...
        uintptr_t iltRVA = desc->OriginalFirstThunk;
        ...
        for (DWORD ti = 0; ; ti++) {
            ...
            if (funcAddr) {
                uintptr_t remoteIAT = baseVA + iatRVA + ti * sizeof(uintptr_t);
                WriteProcessMemory(hProcess, (void*)remoteIAT, &funcAddr, sizeof(void*), nullptr);
            }
        }
    }
    return true;
